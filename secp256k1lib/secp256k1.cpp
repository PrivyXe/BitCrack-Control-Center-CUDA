...
uint256 uint256::sub(int val) const
{
	...
	// Bug #14: ::sub returns borrow indicator; explicit assignment documents intentional wrap-around
	int borrow = ::sub(this->v, result.v, result.v, 8);
	(void)borrow;
...
{
	// Bug #9 fix: explicitly use 0x00000001 hex literal instead of octal 00000001
	unsigned int barrettN[] = { 0x2fc9bec0, 0x402da173, 0x50b75fc4, 0x45512319, 0x00000001, 0x00000000, 0x00000000, 0x00000000, 0x00000001 };
...
/**
 * Parses a public key.
 * Accepts uncompressed (04 + 64 hex X + 64 hex Y) or compressed (02/03 + 64 hex X).
 */
secp256k1::ecpoint secp256k1::parsePublicKey(const std::string &pubKeyString)
{
	std::string s = pubKeyString;
	...
	if(s.length() == 66 && (s[0] == '0') && (s[1] == '2' || s[1] == '3')) {
		...
		// y^2 = x^3 + 7 (mod P); P % 4 == 3 => y = +/- (y2)^((P+1)/4)
		uint256 y2 = addModP(multiplyModP(multiplyModP(x, x), x), uint256(7));
		uint256 exp("3fffffffffffffffffffffffffffffffffffffffffffffffffffffffbfffff0c");

		uint256 y(1);
		uint256 base = y2;
		for(int i = 0; i < 256; i++) {
			if(exp.bit(i)) {
				y = multiplyModP(y, base);
			}
			base = multiplyModP(base, base);
		}
