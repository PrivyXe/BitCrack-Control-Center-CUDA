...
struct RunConfig {
    ...
    bool follow = false;
    bool kangarooMode = false;
    unsigned int dpBits = 16;
    bool hasPubKey = false;
    secp256k1::ecpoint targetPubKey;
};
...
void statusCallback(KeySearchStatus info)
{
	...
	} else if(info.speed >= 1000.0) {
		speedStr = util::format("%.2f", info.speed / 1000.0) + " GKey/s";
	} else {
...
void usage()
{
    ...
    printf("-k, --kangaroo          Use Pollard's Kangaroo algorithm (requires --pubkey)\n");
    printf("-dp, --dp BITS          Distinguished point bits for kangaroo (default 16)\n");
    printf("--pubkey KEY            Target public key (02/03 compressed or 04 uncompressed hex)\n");
}
...
void writeCheckpoint(secp256k1::uint256 nextKey)
{
    if(_config.checkpointFile.empty()) return;

    std::string tmpFileName = _config.checkpointFile + ".tmp";
    std::ofstream tmp(tmpFileName, std::ios::out);
    ...
    if(tmp.good()) {
        std::remove(_config.checkpointFile.c_str());
        std::rename(tmpFileName.c_str(), _config.checkpointFile.c_str());
    }
}
...
int run()
{
    ...
    if(_config.kangarooMode) {
        Logger::log(LogLevel::Info, "Mode:        Kangaroo (Pollard)");
        Logger::log(LogLevel::Info, "DP bits:     " + util::format(_config.dpBits));
    } else {
    ...
    try {
        ...
        std::unique_ptr<KeySearchDevice> d;

        if(_config.kangarooMode) {
#ifdef BUILD_CUDA
            if(_devices[_config.device].type != DeviceManager::DeviceType::CUDA) {
                Logger::log(LogLevel::Error, "Kangaroo mode requires a CUDA device");
                return 1;
            }
            if(!_config.hasPubKey) {
                Logger::log(LogLevel::Error, "Kangaroo mode requires --pubkey <hex>");
                return 1;
            }

            CudaKangarooDevice *kd = new CudaKangarooDevice(
                (int)_devices[_config.device].physicalId,
                (int)_config.threads,
                (int)_config.blocks,
                _config.dpBits,
                (int)_config.pointsPerThread);
            kd->setTargetPublicKey(_config.targetPubKey);
            kd->setKeyRange(_config.nextKey, _config.endKey);
            d.reset(kd);
#else
            Logger::log(LogLevel::Error, "Kangaroo mode requires CUDA build");
            return 1;
#endif
        } else {
            d.reset(getDeviceContext(_devices[_config.device], _config.blocks, _config.threads, _config.pointsPerThread));
        }

        if(!d) {
            Logger::log(LogLevel::Error, "Failed to create device context");
            return 1;
        }

        KeyFinder f(_config.nextKey, _config.endKey, _config.compression, d.get(), _config.stride);
        ...
        if(!_config.targetsFile.empty()) {
            ...
            // If only pubkey was provided, derive an address target for result reporting
            if(_config.targets.empty() && _config.hasPubKey) {
                bool compressed = _config.compression != PointCompressionType::UNCOMPRESSED;
                _config.targets.push_back(Address::fromPublicKey(_config.targetPubKey, compressed));
            }
...
int main(int argc, char **argv)
{
    ...
    parser.add("-k", "--kangaroo", false);
    parser.add("-a", "--algo", true);
    parser.add("", "--dp", true);
    parser.add("-dp", "--dp", true);
	...
	for(unsigned int i = 0; i < args.size(); i++) {
		...
		try {
			if(optArg.equals("-t", "--threads")) {
                ...
                _config.follow = true;
            } else if(optArg.equals("-k", "--kangaroo")) {
                _config.kangarooMode = true;
            } else if(optArg.equals("-a", "--algo")) {
                if(optArg.arg == "kangaroo" || optArg.arg == "Kangaroo") {
                    _config.kangarooMode = true;
                }
            } else if(optArg.equals("", "--dp") || optArg.equals("-dp", "--dp")) {
                _config.dpBits = util::parseUInt32(optArg.arg);
            } else if(optArg.equals("", "--pubkey")) {
                try {
                    _config.targetPubKey = secp256k1::parsePublicKey(optArg.arg);
                    _config.hasPubKey = true;
                } catch(std::string err) {
