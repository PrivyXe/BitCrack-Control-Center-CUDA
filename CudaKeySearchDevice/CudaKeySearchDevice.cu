...
{
    __shared__ unsigned int s_incX[8];
    __shared__ unsigned int s_incY[8];

    if (threadIdx.x < 8) {
        s_incX[threadIdx.x] = _INC_X[threadIdx.x];
        s_incY[threadIdx.x] = _INC_Y[threadIdx.x];
    }
    __syncthreads();
    ...
    for(int i = 0; i < pointsPerThread; i++) {
        ...
        beginBatchAdd(s_incX, x, chain, i, i, inverse);
    }
    ...
    for(int i = pointsPerThread - 1; i >= 0; i--) {
        ...
        completeBatchAdd(s_incX, s_incY, xPtr, yPtr, i, i, chain, inverse, newX, newY);
...
__device__ void doIterationWithDouble(int pointsPerThread, int compression)
{
    ...
    for(int i = 0; i < pointsPerThread; i++) {
        ...
        beginBatchAddWithDouble(s_incX, s_incY, xPtr, chain, i, i, inverse);
    }
    ...
    for(int i = pointsPerThread - 1; i >= 0; i--) {
        ...
        completeBatchAddWithDouble(s_incX, s_incY, xPtr, yPtr, i, i, chain, inverse, newX, newY);
