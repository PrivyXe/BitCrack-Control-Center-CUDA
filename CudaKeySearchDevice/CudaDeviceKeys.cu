...
{
	size_t numPoints = (size_t)_threads * _blocks * _pointsPerThread;

	std::vector<unsigned int> xBuf(numPoints * 8);
	std::vector<unsigned int> yBuf(numPoints * 8);

	cudaError_t err = cudaMemcpy(xBuf.data(), _devX, sizeof(unsigned int) * 8 * numPoints, cudaMemcpyDeviceToHost);
	if(err != cudaSuccess) {
		throw std::string("selfTest failed: cudaMemcpy X error: ") + cudaGetErrorString(err);
	}

	err = cudaMemcpy(yBuf.data(), _devY, sizeof(unsigned int) * 8 * numPoints, cudaMemcpyDeviceToHost);
	if(err != cudaSuccess) {
		throw std::string("selfTest failed: cudaMemcpy Y error: ") + cudaGetErrorString(err);
	}
	...
	for(int block = 0; block < _blocks; block++) {
		for(int thread = 0; thread < _threads; thread++) {
			for(int idx = 0; idx < _pointsPerThread; idx++) {
				...
				secp256k1::uint256 x = readBigInt(xBuf.data(), block, thread, idx);
				secp256k1::uint256 y = readBigInt(yBuf.data(), block, thread, idx);
