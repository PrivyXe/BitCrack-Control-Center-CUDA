...
uint64_t CudaKeySearchDevice::getPrivateKeyOffset(uint64_t thread, uint64_t block, uint64_t idx)
{
    // Total number of threads
    uint64_t totalThreads = (uint64_t)_blocks * (uint64_t)_threads;

    uint64_t base = idx * totalThreads;

    // Global ID of the current thread
    uint64_t threadId = block * (uint64_t)_threads + thread;
...
void CudaKeySearchDevice::getResultsInternal()
{
    ...
    uint64_t pointsPerStep = (uint64_t)_blocks * (uint64_t)_threads * (uint64_t)_pointsPerThread;
    ...
    for(int i = 0; i < count; i++) {
        ...
        secp256k1::uint256 stepOffset = secp256k1::uint256(pointsPerStep) * secp256k1::uint256(_iterations);
        secp256k1::uint256 keyOffset = stepOffset + secp256k1::uint256(getPrivateKeyOffset(rPtr->thread, rPtr->block, rPtr->idx));
        secp256k1::uint256 offset = keyOffset * _stride;
...
{
    uint64_t totalPoints = (uint64_t)_pointsPerThread * (uint64_t)_threads * (uint64_t)_blocks;

    return _startExponent + secp256k1::uint256(totalPoints) * secp256k1::uint256(_iterations) * _stride;
}