...
void atomicListAdd(__global CLDeviceResult *results, __global unsigned int *numResults, unsigned int maxResults, CLDeviceResult *r)
{
...
void setResultFound(int idx, bool compressed, uint256_t x, uint256_t y, unsigned int digest[5], __global CLDeviceResult* results, __global unsigned int* numResults, unsigned int maxResults)
{
    CLDeviceResult r;
    ...
    for(; i < totalPoints; i += dim) {
        ...
        if((compression == UNCOMPRESSED) || (compression == BOTH)) {
            ...
            if(checkHash(digest, targetList, numTargets, mask)) {
                setResultFound(i, false, x, y, digest, results, numResults, maxResults);
            }
        }
        ...
        if((compression == COMPRESSED) || (compression == BOTH)) {
            ...
            if(checkHash(digest, targetList, numTargets, mask)) {
                uint256_t y = yPtr[i];
                setResultFound(i, true, x, y, digest, results, numResults, maxResults);
            }
        }
...
__kernel void keyFinderKernel(
    ...
    unsigned int maxResults)
{
    doIteration(totalPoints, compression, chain, xPtr, yPtr, incXPtr, incYPtr, targetList, numTargets, mask, results, numResults, maxResults);
}
...
__kernel void keyFinderKernelWithDouble(
    ...
    unsigned int maxResults)
{
    doIterationWithDouble(totalPoints, compression, chain, xPtr, yPtr, incXPtr, incYPtr, targetList, numTargets, mask, results, numResults, maxResults);
}
